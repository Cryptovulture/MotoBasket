/**
 * Monitor NAV of all 10 indexes vs MOTO over time.
 * Prints a table every new block showing:
 *  - Current NAV per share (in MOTO)
 *  - Change from initial NAV (%)
 *  - Index price relative to MOTO
 *
 * Usage: node monitor-nav.mjs [interval_seconds]
 */
...
const INDEXES = [
  { symbol: 'OPAI', addr: '0x1c625456bc60c28d590dc2419ba649a4a8215a21a51454c637410ea580232eb8',
    components: [
      '0xaaf45d2ce96330c48a86f6e45d1f666d38f84c39c3cbb0db379ce806a91ed86f',
  ...
  { symbol: 'OPMEME', addr: '0x2f8c7fb147949a5bc95f6832a1c1ae7446ced4c5a3a7b3270160a3dc5a5836dd',
    components: [
      '0x2495973eca6972620d9565e4236bd966d8723fb36911c0e3e84bc6ef6201c2d6',
      '0x24550d41f446261c3091fb2dad64a62f9065398650e238e2338a22b3bd7a2e22',
      '0x07ec6eb7dd1c071053d390053ea60dfad0a39dc86f84bbd5ba95d7d858789258',
      '0x274cca66f076864b97a29e1001275c69d48eaad9c4eda460f58914e005f8f383',
    ]},
  { symbol: 'OPDEFI', addr: '0xae2575d734f9e6dcba8d2dc8534497b0eed4d563b4ef6360075b2a135400eead',
    components: [
      '0xb3640fd16d44469af46ec74097917d3cf16feb28715b8cd8304ce09245d66c18',
  ...
  { symbol: 'OPGAME', addr: '0x0e1a2cc9f3c32c6ed08a50f5fa9f0c6fa13624e44d15f932c453c2801e57815c',
    components: [
      '0xd489377d4a9a64ccec90996ad4027fb8b824835b875e7d55d864030fdf4c32ce',
      '0x3f8661627f0d0570f5a3c51be4b80e6e3b5e3e71e1f8f8faa567645987345c27',
      '0x1694edcd0df9053bb9fe8b1ccef3057451a08426e888acb9cf2fe9306f0f46fd',
      '0x3ca2249304ce3ac49c8cadc633f3cc5e02896fc46d30ab44fb91d974ed18f9c1',
    ]},
  { symbol: 'OPINFRA', addr: '0xcc5a01846610a268856b0677ad0550a42f9d81fe04ed757fc39595bd6965bf2c',
    components: [
      ...
      '0x4332dafd738b89df51e0c75fb8a1d303e6b542b76f014b2daa4f93d5aabc6d53',
...
// Get pair reserves → returns [motoReserve, tokenReserve]
async function getPairReserves(pairAddr, tokenAddr) {
  ...
  // Determine which is MOTO (token0 = min address)
  const motoClean = MOTO.replace(/^0x/, '').toLowerCase();
  const tokenClean = tokenAddr.replace(/^0x/, '').toLowerCase();
  const motoIsToken0 = motoClean < tokenClean;

  return motoIsToken0 ? [r0, r1] : [r1, r0];
}

// Get token balance held by an index contract
...
// Get total supply of index token
...
// Calculate NAV per share for an index (in MOTO terms)
async function calcNAV(index) {
  const supply = await getTotalSupply(index.addr);
  if (supply === 0n) return { nav: 0, supply: 0n, holdings: [] };
  ...
  for (const compAddr of index.components) {
    ...
    // Get how much of this token the index holds
    const balance = await getBalance(compAddr, index.addr);

    // Get pair reserves to determine MOTO value
    const [motoRes, tokenRes] = await getPairReserves(pairAddr, compAddr);

    // Value of holdings in MOTO: (balance * motoReserve) / tokenReserve
    let motoValue = 0n;
    if (tokenRes > 0n && balance > 0n) {
      motoValue = (balance * motoRes) / tokenRes;
    }

    totalMotoValue += motoValue;
    holdings.push({
      token: compAddr.substring(0, 10) + '...',
      balance: Number(balance) / 1e18,
      motoValue: Number(motoValue) / 1e18,
    });
  }

  const navPerShare = Number(totalMotoValue) / Number(supply);
  return { nav: navPerShare, supply, holdings, totalMotoValue: Number(totalMotoValue) / 1e18 };
}
...
async function printStatus() {
  ...
  console.log(`\n╔══════════════════════════════════════════════════════════════╗`);
  console.log(`║  MotoBasket NAV Monitor — Block ${block} — ${time}  `);
  console.log(`╠══════════════════════════════════════════════════════════════╣`);
  console.log(`║  Index    │  NAV/Share  │  Total Value │  Change  │ Supply  ║`);
  console.log(`╠══════════════════════════════════════════════════════════════╣`);

  for (const idx of INDEXES) {
    try {
      const { nav, supply, totalMotoValue } = await calcNAV(idx);

      if (supply === 0n) {
        console.log(`║  ${idx.symbol.padEnd(8)} │  (empty)    │     0.00     │    —     │   0.00  ║`);
        continue;
      }

      // Track initial NAV
      if (!initialNavs[idx.symbol]) {
        initialNavs[idx.symbol] = nav;
      }

      const change = initialNavs[idx.symbol] > 0
        ? ((nav - initialNavs[idx.symbol]) / initialNavs[idx.symbol] * 100)
        : 0;

      const changeStr = change >= 0 ? `+${change.toFixed(2)}%` : `${change.toFixed(2)}%`;
      const changeColor = change > 0 ? '\x1b[32m' : change < 0 ? '\x1b[31m' : '\x1b[0m';

      console.log(`║  ${idx.symbol.padEnd(8)} │  ${nav.toFixed(4).padStart(9)}  │ ${totalMotoValue.toFixed(2).padStart(11)} M │ ${changeColor}${changeStr.padStart(7)}\x1b[0m  │ ${(Number(supply)/1e18).toFixed(1).padStart(6)}  ║`);
    } catch (e) {
...
async function main() {
  ...
  console.log(`Starting NAV monitor (checking every ${interval}s)...`);
  ...
  // Loop
  let lastBlock = parseInt(await rpc('btc_blockNumber', []), 16);
  while (true) {
    await new Promise(r => setTimeout(r, interval * 1000));
    const currentBlock = parseInt(await rpc('btc_blockNumber', []), 16);
    if (currentBlock !== lastBlock) {
      lastBlock = currentBlock;
      await printStatus();
    } else {
      process.stdout.write(`\r  Block ${currentBlock} — no change — ${new Date().toLocaleTimeString()}  `);
    }
  }
}
